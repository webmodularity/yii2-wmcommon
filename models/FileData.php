 ...
 */
class FileData extends \wmc\db\ActiveRecord
{
    ...
    public function getFullName() {
        return $this->name . '.' . $this->fileType->primaryExtension->extension;
    }

    public function getFullAlias() {
        return $this->alias . '.' . $this->fileType->primaryExtension->extension;
    }
    ...
    public function afterDelete() {
        ...
        if (!empty($filePath) && !empty($fileType)) {
            $filename = Yii::getAlias($filePath->path) . DIRECTORY_SEPARATOR . $this->fullName;
            @unlink($filename);
        }
        parent::afterDelete();
    }
    ...
    public function beforeSave($insert) {
        if (parent::beforeSave($insert)) {
            ...
            if ($insert) {
                $extension = FileTypeExtension::find()
                    ->where([
    ...
    public function afterSave($insert, $changedAttributes) {
        if (!$insert) {
            ...
            if (in_array('file_path_id', array_keys($changedAttributes))) {
                $newFileName = static::normalizeFileName($this->name, $this->fileType->extension, $this->filePath->path);
                $oldFilePath = FilePath::findOne($changedAttributes['file_path_id']);
                $oldFullPath = Yii::getAlias($oldFilePath->path) . DIRECTORY_SEPARATOR . $this->name . '.' . $this->fileType->extension;
                $newFullPath = Yii::getAlias($this->filePath->path) . DIRECTORY_SEPARATOR . $newFileName . '.' . $this->fileType->extension;
                if (rename($oldFullPath, $newFullPath)) {
                    ...
                    if ($this->name != $newFileName) {
                        ...
                        if (!$this->save(true, ['name'])) {
                            ...
                            $this->file_path_id = $changedAttributes['file_path_id'];
